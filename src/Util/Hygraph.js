...
const endpoint =
  "https://api-eu-west-2.hygraph.com/v2/cl7oqf56l3imn01t734v4f3r6/master";
...
export async function getLogo(onSuccess) {
  ...
  try {
    ...
    console.log(
      error + "A logo is required. Please add the content and try again."
    );
  }
}
...
export async function getIcon(onSuccess) {
  ...
  try {
    ...
    console.log(
      error + "An icon is required. Please add the content and try again."
    );
  }
}
...
export async function getArtists(onSuccess) {
  const query = gql`
    query artists {
      artists(first: 100, orderBy: name_ASC) {
        slug
        name
        nameLabel {
          url
